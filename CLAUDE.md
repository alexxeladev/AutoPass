...
| # | Описание | Причина |
|---|----------|---------|
| 1 | Авто не сохраняются при добавлении нового жильца через модал | Неверный CSS-селектор `#addResidentModal` вместо `#addModal` в submit-обработчике (residents.html) |
...
- **Баг исправлен:** авто при создании жильца теперь сохраняются (починен селектор `#addModal` в submit-обработчике)
